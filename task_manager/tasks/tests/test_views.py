...
class ViewTests(TestCase):
    ...
    def test_filter_by_status(self):
        ...
        response = self.client.get(reverse('project_detail', args=[self.project.id]), {'status': 'done'})
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Implement feature")
    ...
    def test_search_by_title(self):
        ...
        response = self.client.get(reverse('project_detail', args=[self.project.id]), {'search': 'bug'})
        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "Fix bug")
    ...
    def test_sort_by_due_date(self):
        ...
        response = self.client.get(reverse('project_detail', args=[self.project.id]), {'ordering': 'due_date'})
        self.assertEqual(response.status_code, 200)
        tasks = response.context['page_obj']
        self.assertEqual(tasks[0].title, "Fix bug")  # Первая задача должна быть "Fix bug"
        self.assertEqual(tasks[1].title, "Implement feature")  # Вторая задача должна быть "Implement feature"