...
from tasks.models import Comment, Project, Task
...
class ModelTests(TestCase):
    ...
    def test_project_task_count(self):
        ...
        Task.objects.create(
            project=self.project, title="Task 1", status="todo", user=self.user
        )
        Task.objects.create(
            project=self.project, title="Task 2", status="in_progress", user=self.user
        )
    ...
    def test_project_unique_name_per_user(self):
        ...
        with self.assertRaises(
            Exception
        ):  # Или конкретное исключение, например, IntegrityError
    ...
    def test_task_str_representation(self):
        ...
        task = Task.objects.create(
            project=self.project, title="Test Task", status="todo", user=self.user
        )
    ...
    def test_comment_creation(self):
        ...
        comment = Comment.objects.create(task=task, user=self.user, text="Test comment")
    ...
    def test_comment_str_representation(self):
        ...
        comment = Comment.objects.create(task=task, user=self.user, text="Test comment")
        self.assertEqual(
            str(comment), f"Comment by {self.user.username} on {task.title}"
        )
